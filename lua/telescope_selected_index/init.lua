...
local _config = require("telescope_selected_index.config")

local _patched = false       -- Avoid double-patching
local _original_pickers = {} -- store original functions
		...
		if type(picker) == "function" then
			...
			telescope[name] = function(opts_inner)
				...
				opts_inner.attach_mappings = function(prompt_bufnr, map)
					if orig_attach then
						orig_attach(prompt_bufnr, map)
					end
					return attach_fn(prompt_bufnr, map)
				end
				return _original_pickers[name](opts_inner)
			end
		end
	end
end

--- Restore original telescope functions
function M.unpatch()
	local ok, telescope = pcall(require, "telescope.builtin")
	if not ok then
		return
	...
	for name, orig_fn in pairs(_original_pickers) do
		telescope[name] = orig_fn
	end
...
function M.setup(opts)
	...
	if opts.position then
		_config.setup({ position = opts.position })
	end
